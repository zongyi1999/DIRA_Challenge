...
from fastreid.evaluation.evaluator import inference_context, inference_for_submit
from fastreid.data import build_reid_test_loader, build_reid_train_loader
from fastreid.utils.compute_dist import build_dist
from fastreid.evaluation.reid_evaluation import aqe
...
def main(args):
    ...
    if cfg.TEST.AQE.ENABLED:
        print("Test with AQE setting")
        qe_time = cfg.TEST.AQE.QE_TIME
    ...
    # print(np.min(scores2),np.max(scores2),scores2.shape, scores2.dtype, scores2)
    # scores2 = np.load("/media/backup/competition/eccv_retrieve/result_reid_aqe.npy")
    # print(np.min(scores2),np.max(scores2),scores2.shape, scores2.dtype, scores2)
